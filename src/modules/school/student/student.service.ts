...
export class StudentService {
    ...
    async createStudent(schoolId: string, createStudentDto: CreateStudentDto) {
        ...
        if (existingStudent) {
            throw new ConflictException('Student with this Admission No, ID, or Email already exists');
        }
        ...
        return this.prisma.$transaction(async (tx) => {
            ...
            if (parents && parents.length > 0) {
                for (const parentDto of parents) {
                    ...
                    if (parentInfo.cnic) parentOrConditions.push({ cnic: parentInfo.cnic });
                    if (parentInfo.email) parentOrConditions.push({ email: parentInfo.email });
    ...
    async findAll(schoolId: string, pagination: PaginationParamsDto, filter: StudentFilterDto) {
